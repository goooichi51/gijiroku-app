...
### 2-1. 文字起こし: ローカル処理（WhisperKit）

**WhisperKit**（MIT License）をiOS上でローカル実行し、文字起こしをオンデバイスで完結させる。

| 項目 | 内容 |
|------|------|
| 技術 | WhisperKit（Argmax社、Swift、CoreML + Apple Neural Engine最適化） |
| 日本語精度 | largeモデルで高精度。日本語特化の**Kotoba Whisper V1.0**（large-v3の6.3倍速・同等精度）も利用可能 |
| 話者識別 | **SpeakerKit**（Argmax社、約10MB）を併用。4分音声を約1秒で話者識別 |
| リアルタイム | 対応済み。レイテンシ約0.46秒 |
| 処理速度 | iPhone 15以降で実用的。M2 Ultraで72倍リアルタイム速度（1時間→約1分25秒） |
| モデルサイズ | large-v3: 約1.5GB、圧縮技術で1GB未満に縮小可能。small: 約244MB |
| 動作要件 | iOS 16以降、推奨: iPhone 15以降 |
| コスト | **無料**（APIコール不要） |
| プライバシー | **音声データが端末外に一切出ない** |

**メリット**: ランニングコスト0、完全オフライン文字起こし、プライバシー最強
**デメリット**: 初回モデルダウンロード（数百MB〜1.5GB）、バッテリー消費、古いiPhoneでは性能不足

### 2-2. AI要約: GLM-4.7-Flash API（Zhipu AI）

| 項目 | 内容 |
|------|------|
| テキスト入力 | **$0.06 / 100万トークン** |
| テキスト出力 | **$0.40 / 100万トークン** |
| **1時間の会議要約コスト** | **約0.35円** |
| 日本語品質 | 実用レベル。敬語・コンテキスト処理が自然。JSON出力が安定 |
| 処理速度 | 約180TPS（日本語で秒200〜300文字） |
| コンテキスト | 203Kトークン（長時間会議にも対応） |

**文字起こし（ローカル: 0円）+ 要約（GLM-4.7-Flash: 約0.35円）= 1時間あたり約0.35円**

参考: Gemini 2.5 Flash（$0.30/$2.50）だと約1.9円/時間 → GLM-4.7-Flashは**約5倍安い**
...
| **API Key保護** | GLM-4.7-Flash APIキーをサーバー側に置き、アプリに直接埋め込まない |
...
| レイヤー | 技術 | 理由 |
|----------|------|------|
| 文字起こし | **WhisperKit**（ローカル） | コスト0、プライバシー、オフライン対応 |
| 話者識別 | **SpeakerKit**（ローカル） | 10MB、高速、WhisperKitと同一ベンダー |
| AI要約 | **GLM-4.7-Flash API** | 最安（約0.35円/時間）、日本語対応 |
...
| F3 | AI文字起こし（ローカル） | WhisperKitでオンデバイス文字起こし（日本語高精度） | P0 |
| F4 | 話者識別 | SpeakerKitで発話者を自動分離・ラベリング | P0 |
| F5 | AI議事録要約 | GLM-4.7-Flashで文字起こし→議事録自動生成（要点・決定事項・アクションアイテム） | P0 |
...
| 項目 | 要件 |
|------|------|
| 対応OS | iOS 16以降（WhisperKit要件）、watchOS 10以降 |
| 推奨端末 | iPhone 15以降（largeモデル実行に必要なRAM） |
| 日本語精度 | 95%以上（静かな環境） |
| 録音時間 | 最大4時間/回（iPhone・Watch共通）。Watch録音は30分ごとに継続確認を表示し、5分間応答なしで自動停止 |
| 文字起こし | 録音完了後、端末上で処理（1時間→数分） |
...
**価格設定の根拠:**
- 文字起こしコスト: **0円**（ローカル処理）
- AI要約コスト: **約0.35円/時間**（GLM-4.7-Flash）
- Standardプランで月20時間利用した場合の原価: 約7円 → **粗利率98%以上**
...
| リスク | 影響 | 対策 |
|--------|------|------|
| 古いiPhoneでWhisperKitが重い | 中 | smallモデルへのフォールバック、推奨端末の明記 |
| 初回モデルダウンロードが大きい | 中 | Wi-Fi時に自動DL、進捗表示、smallモデルから開始可能に |
| GLM-4.7-Flash APIの料金変更・サービス停止 | 低 | 原価が極めて低いため吸収可能。代替LLM（Gemini Flash、GPT-4o mini等）に差し替え可能 |
...
4. ~~技術検証計画（WhisperKit + SpeakerKit PoC計画書）~~ ✅ → `docs/poc-plan.md`
5. ~~事業計画（収支シミュレーション）~~ ✅ → `docs/business-plan.md`

開発フェーズ（別プロジェクト）：
- Swift/SwiftUI + watchOSアプリ開発
- Supabase構築（認証・データ同期）
- WhisperKit + SpeakerKit統合
- GLM-4.7-Flash API連携
- PDF生成・共有機能実装
